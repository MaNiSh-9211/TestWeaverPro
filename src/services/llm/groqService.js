...
remember the should continue field is used to control the workflow continuation so if you think that the next step should not be performed then set it to false and if you think that the next step should be performed then set it to true and in teh last etstcase object if you have to check the userstory and crosscheck with the testcase , if you find that all the testcases that user mentioned in userstory are complete then in the last selectors shouldContinue field ad false so that the graph can stop its exicution
        `.trim();
...
never use selectors like -> /html/body/div/div/div/div/form/div[2]/input
Rules:
...
Each object in the response array represents one interaction. Analyze the provided user story and the current HTML content, and return only the selectors for those interactions that are possible to perform on the given HTML. Do not attempt to generate selectors for the entire user story in a single response. After executing the returned interactions, we will send the updated HTML back to you, and the process will repeat. Your response should only include actionable selectors based on the current page content. Ensure the output strictly adheres to the schema and includes all 6 fields, and give the testcase objects in sequesnce because the one at 0 index will be exicuted first so think which intraction must be performed first like filling details before clicking on button.
        . remember the main point if you recieve executionResults array of object then that means those testcases were given and done by you before as you cant remember the state across api calls  so in such case give testcases for next task from userstory dont repeat old ones and read whole userstory and then decide for how must of that you have created testcases aand if anything pending then give shouldcontinue as true`.trim();
